...
fs.readdir('./txt', function(err, filenames) {
  ...
  for (let filename in hood_counts) {
    let sorted_hoods = [];
    for (let hood in hood_counts[filename]) {
      sorted_hoods.push({
        hood: hood,
        count: hood_counts[filename][hood]
      });
    }
    sorted_hoods.sort(function(a,b) {
      return b.count - a.count;
    });
    newStructure[filename] = JSON.parse(JSON.stringify(sorted_hoods));
  }
  
  let buffer = '';
  for (let filename in newStructure) {
    let just_hoods = [];
    for (let hood of newStructure[filename]) {
      just_hoods.push(hood.hood);
    }
    buffer += filename+',"' + just_hoods.join(",") + '"\n';
  }
